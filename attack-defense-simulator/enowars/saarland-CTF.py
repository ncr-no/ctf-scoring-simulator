...
# might not be exact due to a few confusions
	...
	if roundId != previousRoundId:
		for i in range(1, roundId):
			for j in scores["scores"][i]["teams"]:
				...
				defense = 0
				for tmpService, tmpOffset in scores["scores"][i]["teams"][j]["service_defense"].items():
					for flagOffset, tmpLostFlagCount in tmpOffset.items():
						defense -= (((tmpLostFlagCount/numberOfTeams) ** 0.3) * SLAThisRound ) # flagPerRound is not added here because there is only one flag possible for an offset in a service
				...
				for tmpVictimId, tmpVictimFlags in scores["scores"][i]["teams"][j]["service_attack"].items():
					for tmpServiceId, tmpFlagOffsetList in tmpVictimFlags.items():
						for tmpOffset in tmpFlagOffsetList:
							...
							attack += (1 + (1 /tmpCaptureCount) + (1/ scores["scores"][i]["teams"][victimId]["Rank"]))
